	...
	<%	for (int i = 0; i < items.length; i++)
		{
			...
			<!-- RCAAP - FCCN CHANGES LINKS -->
			<h4><a style="text-decoration-color: #000; color: #000;" href="<%= request.getContextPath() %>/handle/<%=items[i].getHandle() %>" class="text-muted">
                        <%= StringUtils.abbreviate(displayTitle, 400) %></a></h4>
                        <!--<%= StringUtils.abbreviate(displayTitle, 400) %></b></a>-->
                        
			<!--<h4><%= StringUtils.abbreviate(displayTitle, 400) %></h4>
		      	<a style="text-decoration: none;" href="<%= request.getContextPath() %>/handle/<%=items[i].getHandle() %>" class="text-muted"><fmt:message key="jsp.recent-submission.see"/></a>
		      -->
			</div>
		    </div>
<%
